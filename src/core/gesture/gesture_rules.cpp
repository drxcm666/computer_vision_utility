...
namespace cvtool::core::gesture
{
    ...
    static bool is_non_thumb_extended(
        const cv::Point2f &mcp, const cv::Point2f &pip, const cv::Point2f &tip, float palm_scale)
    {
    ...
    FingerState extract_finger_state(const cvtool::core::gesture::HandLandmarkResult &data)
    {
        ...
        state.index_extended = is_non_thumb_extended(
            data.points[IndexMcp], data.points[IndexPip], data.points[IndexTip], palm_scale);

        state.middle_extended = is_non_thumb_extended(
            data.points[MiddleMcp], data.points[MiddlePip], data.points[MiddleTip], palm_scale);

        state.ring_extended = is_non_thumb_extended(
            data.points[RingMcp], data.points[RingPip], data.points[RingTip], palm_scale);

        state.pinky_extended = is_non_thumb_extended(
            data.points[PinkyMcp], data.points[PinkyPip], data.points[PinkyTip], palm_scale);

        return state;
    }

    cvtool::core::gesture::GestureID classify_hand_gesture(
